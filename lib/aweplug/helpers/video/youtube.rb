  ...
  module Helpers
    module Video
      class YouTube
        ...
        def add(url, product: nil, push_to_searchisko: true)
          if url =~ YOUTUBE_URL_PATTERN
            ...
            json['items'].each do |v|
              videos << add_video(v, product, push_to_searchisko)
            end
            videos
          end
        end
        ...
        def add_video (json, product, push_to_searchisko)
          page_path = Pathname.new(File.join 'video', 'youtube', "#{json['id']}.html")
          video = Aweplug::Helpers::Video::YouTubeVideo.new json, @site
